...
def to_np_array(slice_array):
    slice_array = (
        (slice_array - np.min(slice_array))
        / (np.max(slice_array) - np.min(slice_array))
        * 255
    )
...
def create_images(files, base_dir, output_dir, exclusion_chance):
    ...
    transforms = Compose(
        [
            ...
            # Resized(
            #     keys=["image", "label"],
            #     spatial_size=(224, 224, 131),
            #     mode=("area", "nearest"),
            # ),
            # RandRotate90d(keys=["image", "label"], prob=0.5),
            # Flipd(keys=["image", "label"], spatial_axis=2),
            ScaleIntensityd(keys=["image"], minv=0.0, maxv=1.0, factor=None),
        ]
    )
    ...
    for image_file, label_file in zip(image_files, label_files):
        ...
        for i in range(image_vol.shape[2]):
            ...
            image_slice_path = os.path.join(
                output_dir, "images", f"{image_slice_filename}"
            )
            label_slice_path = os.path.join(
                output_dir, "labels", f"{label_slice_filename}"
            )
...
def process_nifti_files(
    train_files, test_files, base_dir, output_dir, json_file, exclusion_chance=0.0
):
    ...
    dataset["labels"] = {"0": "background", "1": "mv2"}
